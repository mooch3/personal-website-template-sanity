...
const InnerHealing = () => {
    ...
    const clickHandler = () => {
        setIsScheduling(prevValue => !prevValue)
    };
    ...
    return (
        <>
            ...
            />
            {!isScheduling && <button className={`centered ${classes.button}`} onClick={clickHandler}>Schedule an Appointment</button>}
            {isScheduling && <Calendar/>}
        </>
    )
}
